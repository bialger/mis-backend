...
interface PatientRepository : CrudRepository<PatientEntity, UUID> {
    ...
    @Query("SELECT * FROM patient ORDER BY full_name LIMIT :limit")
    fun findTopOrdered(limit: Int): List<PatientEntity>

    @Query("SELECT * FROM patient WHERE id IN (:ids)")
    fun findByIds(ids: List<UUID>): List<PatientEntity>
